  ...
  try {
    ...
    if (existingCategory) {
      return res.status(400).json({
        message: "Slug sudah digunakan. Silakan gunakan nama/slug lain.",
      });
    }
    ...
    const newCategory = await ProductCategory.create({
      ...
      default_base_price: default_base_price || 0,
      default_sales_price: default_sales_price || 0,
      default_max_price: default_max_price || 0,
      default_length: default_length || 0,
      default_width: default_width || 0,
      default_height: default_height || 0,
      default_weight: default_weight || 0,
      default_volumetric_weight: default_volumetric_weight || 0,
    });
    ...
    res
      .status(500)
      .json({ message: "Gagal membuat kategori: " + error.message });
  }
};
...
exports.update = async (req, res) => {
  try {
    ...
    if (parsedParentId && parsedParentId == categoryId) {
      return res.status(400).json({
        message:
          "Kategori tidak dapat menjadi sub-kategori dari dirinya sendiri.",
      });
    }
    ...
    if (existingCategory) {
      return res
        .status(400)
        .json({ message: "Slug sudah digunakan oleh kategori lain." });
    }
    ...
    const updateData = {
      ...
      default_base_price:
        default_base_price !== undefined
          ? default_base_price
          : category.default_base_price,
      default_sales_price:
        default_sales_price !== undefined
          ? default_sales_price
          : category.default_sales_price,
      default_max_price:
        default_max_price !== undefined
          ? default_max_price
          : category.default_max_price,
      default_length:
        default_length !== undefined ? default_length : category.default_length,
      default_width:
        default_width !== undefined ? default_width : category.default_width,
      default_height:
        default_height !== undefined ? default_height : category.default_height,
      default_weight:
        default_weight !== undefined ? default_weight : category.default_weight,
      default_volumetric_weight:
        default_volumetric_weight !== undefined
          ? default_volumetric_weight
          : category.default_volumetric_weight,
    };

    // 4. Jika parent_id berubah, dan order_index tidak dikirim (atau tidak valid), hitung order_index baru.
    if (
      parsedParentId !== category.parent_id &&
    ...
    res
      .status(500)
      .json({ message: "Gagal mengupdate kategori: " + error.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    ...
    const children = await ProductCategory.count({
...
exports.updateOrder = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Gagal memperbarui urutan: " + error.message });
  }
};
